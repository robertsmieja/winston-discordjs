...
## 2024-11-20 - Unrestricted Discord Mentions via Log Injection
**Vulnerability:** Sending raw string messages or unrestricted payloads to Discord APIs allows malicious logs containing `@everyone` or specific user pings to trigger unwanted notifications.
**Learning:** All logs sent to a notification platform like Discord must explicitly disable mention parsing by default, as logs contain untrusted external input that could lead to notification spam or denial of service through annoying pings.
**Prevention:** Always set `allowedMentions: { parse: [] }` when sending payloads to Discord via its API or webhooks.
