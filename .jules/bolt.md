...
## 2024-05-24 - Performance cost of locale-aware string methods
**Learning:** `toLocaleUpperCase()` has significant performance overhead (~10-15x slower in benchmarks) compared to `toUpperCase()` in Node.js/V8 due to locale-awareness. In hot paths like logging pipelines, this overhead compounds rapidly.
**Action:** Always prefer `toUpperCase()` or `toLowerCase()` in hot paths (like string formatting or logging) to gain a significant performance improvement (~90%), unless locale-specific conversions are explicitly required by the business logic.
