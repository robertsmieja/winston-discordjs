...
**Learning:** In Node.js / V8, locale-aware string methods like `toLocaleUpperCase()` are significantly slower (~15x overhead in microbenchmarks) than standard ASCII-focused methods like `toUpperCase()`.
**Action:** Always prefer `toUpperCase()` over `toLocaleUpperCase()` for string capitalization in hot paths (like logging formatters) unless locale-specific formatting rules are strictly required by the user.
