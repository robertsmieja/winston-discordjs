...
## 2024-05-14 - String case conversion performance
**Learning:** `toUpperCase()` is significantly faster (~90%) than `toLocaleUpperCase()` in Node.js because it avoids locale-aware lookup overhead. While formatting logs, standard uppercase conversion is sufficient and more performant.
**Action:** When capitalizing or uppercasing strings in hot paths like log formatting, prefer `toUpperCase()` unless locale-awareness is strictly required.
