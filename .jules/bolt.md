...
## 2024-06-21 - String conversion optimizations in hot paths
**Learning:** `toLocaleUpperCase()` has significant performance overhead compared to `toUpperCase()` in Node.js/V8 due to locale-awareness. Replacing it in formatting hot paths provides ~15x faster execution.
**Action:** Always prefer `toUpperCase()` over `toLocaleUpperCase()` in hot paths unless locale-specific conversions are explicitly required.
