...
**Learning:** `toLocaleUpperCase()` has significant performance overhead compared to `toUpperCase()` in Node.js/V8 due to locale-awareness.
**Action:** Always prefer `toUpperCase()` or `toLowerCase()` in hot paths (like string formatting or logging) to gain a significant performance improvement (~95% execution time drop), unless locale-specific conversions are explicitly required by the business logic.
