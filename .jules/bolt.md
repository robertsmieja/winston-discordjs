...
**Learning:** `toLocaleUpperCase()` has significant performance overhead (~4x to 15x slower) compared to `toUpperCase()` in Node.js/V8 due to locale-awareness. In hot paths (like string formatting or logging in `LogHandlers.ts`), using it causes unnecessary CPU overhead.
